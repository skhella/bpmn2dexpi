...
import { importDexpiWithElk } from './transformer/DexpiImportWithElk';
...
function App() {
  ...
  // Import DEXPI XML). DEXPI files are converted to BPMN on import via
  // importDexpiWithElk (importer + ELK relayout), so both sources land
  // as a BPMN diagram in the modeler.
  ...
  const handleImportDexpi = () => {
    ...
    input.onchange = async (e: any) => {
      ...
      try {
        ...
        // DEXPI → BPMN: the importer produces BPMN with its own heuristic
        // layout, then ELK overrides the layout for a readable PFD.
        const bpmnXml = await importDexpiWithElk(dexpiXml);
              ...
              <>
                ...
                >
                  ...
                  <button
                    ...
                    className="btn"
                    style={{ textAlign: 'left' }}
                    title="Import a DEXPI 2.0 XML file and convert it to BPMN (with ELK layout)"
                  >
                    Import DEXPI XML
                  </button>
                </div>

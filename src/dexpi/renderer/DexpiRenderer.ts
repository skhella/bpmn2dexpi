...
export default class DexpiRenderer extends BaseRenderer {
  ...
  private calculatePortPositionFromConnection(
    ...
    if (port.portType === 'InformationPort' || (port as any).type === 'InformationPort') {
      // Live association waypoints take precedence so the port follows the
      // association in real time. Stored anchorX/Y is only used as a fallback
      // when no association exists (e.g. after manual port drag on an
      // unconnected port). InformationPorts only render when a visual
      // association connection exists; if no matching DataObject association
      // is found, return null → port is not drawn (prevents export-only IPI
      // ports from cluttering subprocess boundaries).
